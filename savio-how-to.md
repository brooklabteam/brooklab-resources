...
16. Building on above, note that the different partitions get charged at different rates (see [here](https://docs-research-it.berkeley.edu/services/high-performance-computing/user-guide/running-your-jobs/scheduler-config/) again) - savio3_htc, for example, is charged at a rate of 2.67 SUs per core hour while savio3 is charged at a rate of only 1 SU per core hour. This is important to realize in selecting which partition to use, as we obviously prefer to spend fewer SUs at a time. If you need to run a large array job (as mentioned above in #15), it might be more efficient to run it in parallel using [GNU parallel](https://docs-research-it.berkeley.edu/services/high-performance-computing/user-guide/running-your-jobs/gnu-parallel/) on a single node partition (e.g. savio3) rather than on the htp nodes. It is best to test and trial and get consulting advice from the BRC and your lab before you kick off anything that could be costly.
